package org.jpwilliamson.multiplex.menu;
...
import org.jpwilliamson.multiplex.model.ArenaClass;
	...
	private final class ClassMenu extends Menu {
		...
		private ClassMenu(final ArenaClass arenaClass) {
			...
			this.permissionButton = new ButtonConversation(new PermissionPrompt(), ItemCreator.of(
					...
					"Select permission",
					"",
					"Current: " + Common.getOrSupply(arenaClass.getPermission(), "none"),
					"",
					"Set the permission",
					"to use this class."));
		...
		private final class PermissionPrompt extends SimplePrompt {
			...
			@Override
			protected String getPrompt(final ConversationContext ctx) {
				return "&6Enter the permission to use class " + arenaClass.getName() + " or 'default' to remove. Current: &7" + Common.getOrSupply(arenaClass.getPermission(), "none");
			}
		...
		public final class TierMenu extends Menu {
			 ...
			 * The tier class
			 */
			private final ArenaClass.ArenaClassTier tier;
			...
			private TierMenu(final int tierLevel) {
				...
				this.priceButton = Button.makeDecimalPrompt(ItemCreator.of(
						...
						// The action on entering the price
			 ...
			 *
			 */
			private ArenaClass.ArenaClassTier getOrMakeTier(final int tierLevel) {
				ArenaClass.ArenaClassTier tier = arenaClass.getTier(tierLevel);
