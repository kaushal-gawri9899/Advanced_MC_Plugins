package org.jpwilliamson.multiplex.model.tntrun.tools;
...
import org.jpwilliamson.multiplex.model.tntrun.TNTRunArena;
import org.jpwilliamson.multiplex.model.tntrun.TNTRunSettings;
...
public class TNTLevelTool extends VisualTool<TNTRunArena> {
	...
	protected void handleBlockClick(Player player, TNTRunArena arena, ClickType click, Block block) {
		...
		if(settings.setLevelLocation(location.subtract(arena.getReferenceLocation())))
	...
	protected List<Location> getVisualizedPoints(TNTRunArena arena) {
		return Collections.singletonList(arena.getSettings().getLevelLocation().add(arena.getReferenceLocation()));
	}
