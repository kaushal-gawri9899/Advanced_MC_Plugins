package org.jpwilliamson.multiplex.model.kingOfHill.tools;
...
import org.jpwilliamson.multiplex.model.kingOfHill.KingOfHillSettings;
...
// * for {@link KingOfHillArena}
// */
public class ToolHillLocation extends VisualTool<KingOfHillArena> {

	@Getter
	private static final Tool instance = new ToolHillLocation();

	/**
	 * Create new tool for the arena type
	 */
	private ToolHillLocation() {
		super(KingOfHillArena.class);
	}
...
//	 */
	@Override
	protected String getBlockName(final Block block, final Player player, final KingOfHillArena arena) {
...
//	 */
	@Override
	protected CompMaterial getBlockMask(final Block block, final Player player, final KingOfHillArena arena) {
		final ArenaTeam team = findTeam(block.getLocation(), arena);
...
//	 */
	@Override
	protected void handleBlockClick(final Player player, final KingOfHillArena arena, final ClickType click, final Block block) {
		...
		final KingOfHillSettings settings = arena.getSettings();
		...
		settings.setCrystal(team, block.getLocation().subtract(arena.getReferenceLocation()));
...
//	 */
	@Override
	protected void handleAirClick(final Player player, final KingOfHillArena arena, final ClickType click) {
	 ...
	 * @return
	 */
	private ArenaTeam findTeam(final Location location, final KingOfHillArena arena) {
...
//	 */
	@Override
	protected List<Location> getVisualizedPoints(final KingOfHillArena arena) {
//		//return Arrays.asList(arena.getSettings().getCrystals().add(arena.getReferenceLocation()));
//		Location hillLocation = arena.getSettings().getCrystals().get(0).add(arena.getReferenceLocation());
//		return Arrays.asList(hillLocation);

		List<Location> updateLocationList = (arena.getSettings()).getCrystals();
