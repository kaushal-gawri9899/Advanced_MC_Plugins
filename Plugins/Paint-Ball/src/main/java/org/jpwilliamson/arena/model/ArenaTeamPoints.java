...
//		ArenaTeam neutralTeam = ArenaTeam.loadOrCreateTeam("Neutral");
//		if(settings.getArena().getType().equalsIgnoreCase("paintball"))
//		{
//
//			for (final Map.Entry<String, Object> entry : points.entrySet())
//				if (entry.getValue().equals(neutralTeam.getName())) {
//					points.remove(entry.getKey());
//
//					break;
//				}
//
//			points.put(SerializeUtil.serializeLoc(location), neutralTeam.getName());
//			settings.save();
//		}
//		else {
			for (final Map.Entry<String, Object> entry : points.entrySet())
