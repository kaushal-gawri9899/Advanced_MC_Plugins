...
public class ToolEntrancePoints extends VisualTool<Arena> {
	...
	protected void handleBlockClick(final Player player, final Arena arena, final ClickType click, final Block block) {
		if(arena.getType().equalsIgnoreCase("sumo")){
			final SumoSettings settings = (SumoSettings) arena.getSettings();
			final Location old = settings.getEntranceLocations().find(block.getLocation());
			int left = MathUtil.range(settings.getMaxPlayers() - settings.getEntranceLocations().size(), 0, Integer.MAX_VALUE);
			...
			final boolean added = settings.getEntranceLocations().toggle(block.getLocation());
		...
		else if (arena.getType().equalsIgnoreCase("tnt_tag")){
			final TntTagSettings settings = (TntTagSettings) arena.getSettings();
	...
	protected List<Location> getVisualizedPoints(final Arena arena) {
		if(arena.getType().equalsIgnoreCase("sumo"))
			return ((SumoSettings)arena.getSettings()).getEntranceLocations().getLocations();
	...
	public boolean isApplicable(Arena arena) {
		if(arena.getType().equalsIgnoreCase("sumo"))
			return super.isApplicable(arena) && arena.getSettings() instanceof SumoSettings;
