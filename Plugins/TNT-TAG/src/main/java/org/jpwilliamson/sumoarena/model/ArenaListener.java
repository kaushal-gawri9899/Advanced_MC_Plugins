...
import org.jpwilliamson.sumoarena.mysql.ArenaDatabase;
import org.jpwilliamson.sumoarena.util.ArenaUtil;
...
public final class ArenaListener implements Listener {
	...
	public void onQuit(final PlayerQuitEvent event) {
		...
		Common.runLaterAsync(() -> ArenaDatabase.save(player));
	}
	...
	public void onInventoryClick(InventoryClickEvent event) {
		...
		if (cache.hasArena() && cache.getArena() instanceof TntTagArena){

			for(Map.Entry<UUID,Boolean> entry : ((TntTagArena)(cache.getArena())).playersTag.entrySet()){
					if(entry.getKey().equals(player.getUniqueId()) && entry.getValue()){
						event.setCancelled(true);
					}
			}
			}
