...
import org.jpwilliamson.arena.mysql.ArenaDatabase;
import org.jpwilliamson.arena.util.ArenaUtil;
...
public final class ArenaListener implements Listener {
	...
	public void onInventoryClick(InventoryClickEvent event) {
		...
		if(item!=null){
			if(item.getType().equals(CompMaterial.DIAMOND_SWORD) && cache.getArena().getType().equalsIgnoreCase("skywars")){
				event.setCancelled(true);
				int count=0;
				while(count<15){
					SkyWarsArena arena = (SkyWarsArena) cache.getArena();
					//SpinRewardMenu.updateState(cache.getPlayer());
					SpinRewardMenu.updateState(player);
					count++;
				}
